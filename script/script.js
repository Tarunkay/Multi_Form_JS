...
function validateForm() {
    const name = inputInfo.item(0).value.trim();
    const email = inputInfo.item(1).value.trim();
    const phone = inputInfo.item(2).value.trim();

    let isValid = true;

    if (name === "") {
        inputInfo.item(0).classList.add("input-error");
        isValid = false;
    } else {
        inputInfo.item(0).classList.remove("input-error");
    }

    if (!validateEmail(email)) {
        inputInfo.item(1).classList.add("input-error");
        isValid = false;
    } else {
        inputInfo.item(1).classList.remove("input-error");
    }

    if (!validatePhoneNumber(phone)) {
        inputInfo.item(2).classList.add("input-error");
        isValid = false;
    } else {
        inputInfo.item(2).classList.remove("input-error");
    }
...
nextBtnOne.forEach(btn => {
    btn.addEventListener("click", function () {
        if (!validateForm()) {
            fieldMsg.classList.add("show");
            fieldMsg.innerText = "Please fill out all fields correctly.";
        } else {
            fieldMsg.classList.remove("show");
            stepTwoColor();
            stepOneDiv.classList.toggle("hide");
